...
class _SocoLiveDetailScreenState extends State<SocoLiveDetailScreen> {
  ...
  void initState() {
    ...
    AnalyticsUtil.logScreenView(screenName: 'SocoLiveDetailScreen');
    super.initState();
  }
...
class SocoLiveLinkItem extends StatelessWidget {
  const SocoLiveLinkItem({super.key, required this.link});
  ...
  Widget build(BuildContext context) {
    final resolutionIcon =
        link.name.toLowerCase().contains('hd')
            ? Icons.hd_outlined
            : Icons.sd_outlined;
    ...
    return ListTile(
      ...
      leading: CircleAvatar(child: Icon(resolutionIcon)),
